from bs4 import BeautifulSoup
import requests
import urllib.parse

r = requests.get('https://wikipedia.org/wiki/cunt') # hent url med en webhandler

bs = BeautifulSoup(r.text, features='lxml') # lav

titles = [] # laver tom liste
urls = [] # laver tom liste


for i in bs.find_all('p'): # find alle <p> tags på en wikipedia-side
    for j in i.find_all('a'): # find alle <a> tags i <p> tags
        li = j.get('href') # find href værdien i alle <a> tags

        title = j.get('title') # laver en variable der fetcher titlen på linket
        links = 'https://wikipedia.org/wiki' + li # laver en variable der indeholder det fulde link

        if (j.get('title') != None): # sorterer alle links som ikke har en titel fra
            urls.append(links)
        if j.get('title')!= None : # er det andet if statement nødvendigt?
            titles.append(title)


print('URL\'s  :  ' + str((len(urls)))) # print længde af liste
print('Titles :  ' + str((len(titles))) + '\n') # print længde af liste


# print -->
# titles[1], urls[1],
# tiles[2], urls[2]
# ...
for x, y in zip(titles, urls):
    urllib.parse.unquote(y)
    print(x + ' :', y)

#print(urls)
#print(titles)
